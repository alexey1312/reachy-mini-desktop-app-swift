...
extension View {
    ...
    /// tab bar.
    ///
    /// **Apply this to the `TabView`, above `runningAppDock`.** The dock is a
    /// bottom `safeAreaInset`; applied after this modifier it shrinks the safe area
    /// the window reads, so the window comes to rest above the strip instead of on
    /// top of it. The other order puts the overlay outside that inset and the
    /// window lands on the dock's buttons.
    ///
...
private struct FloatingViewportModifier: ViewModifier {
    ...
    /// The safe area, by hand, less the tab bar.
    ///
    /// By hand because `EdgeInsets` is not a `UIEdgeInsets`, so `CGRect` has no
    /// `inset(by:)` that takes one — and less the bar because an overlay on the
    /// `TabView` is outside the inset the bar creates for each tab's content.
    /// A `safeAreaInset` further out *is* reported here, which is how the
    /// running-app dock is allowed for without a constant of its own.
    private func available(in geometry: GeometryProxy) -> CGRect {
        let insets = geometry.safeAreaInsets
        let bottom = insets.bottom + Metrics.tabBarAllowance
        return CGRect(
            x: insets.leading,
            y: insets.top,
            width: max(0, geometry.size.width - insets.leading - insets.trailing),
            height: max(0, geometry.size.height - insets.top - bottom)
        )
    }
}
